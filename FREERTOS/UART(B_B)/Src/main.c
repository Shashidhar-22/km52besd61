...
int main(void)
{
  ...
  osThreadDef(Task1, Task01, osPriorityNormal, 0, 128);
...
void Task01(void const * argument)
{
  ...
  for(;;)
  {
		...
		{
		HAL_UART_Transmit(&huart2,(unsigned char*)"r",1,100);
    osDelay(200);
		}
		if(!(HAL_GPIO_ReadPin(GPIOC,GPIO_PIN_9)))
		{
		HAL_UART_Transmit(&huart2,(unsigned char*)"g",1,100);
    osDelay(200);
		}
	}
  /* USER CODE END 5 */
...
void Task02(void const * argument)
{
  ...
  for(;;)
  {
		HAL_UART_Receive(&huart2,&c,1,200);
			if(c=='r'||c=='R')
			{
