...
void mystrcat( const char* des,char* sor)
{
	int j;
	int k=strlen(des);
	for( j=0;sor[j]!='\0';j++)
...
/*
output:
1.cons_des.c: In function ‘mystrcat’:
1.cons_des.c:32:27: error: assignment of read-only location ‘*(des + ((sizetype)k + (sizetype)j))’
   32 |                 *(des+k+j)=*(sor+j);
      |                           ^
1.cons_des.c:35:19: error: assignment of read-only location ‘*(des + ((sizetype)k + (sizetype)j))’
   35 |         *(des+k+j)='\0';
