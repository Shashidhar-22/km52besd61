...
void mystrcpy(const char* dest,char* sorc)
{
	int i;
	for( i=0;sorc[i]!='\0';i++)
	{
...
/*
output:
1.const_destination.c: In function ‘mystrcpy’:
1.const_destination.c:33:32: error: assignment of read-only location ‘*(dest + (sizetype)i)’
   33 |                         dest[i]=sorc[i];
         |                                ^
	 1.const_destination.c:35:24: error: assignment of read-only location ‘*(dest + (sizetype)i)’
	    35 |                 dest[i]='\0';
	          |                        ^
